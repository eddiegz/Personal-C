        ...
        rep(j,1,w){
            if(grid[i][j]=='.'){
                ...
                while(!to_go.empty()){
                    auto cc=to_go.front();
                    to_go.pop();
                    if(grid[cc.ff-1][cc.ss]!='.'&&grid[cc.ff+1][cc.ss]!='.'&&grid[cc.ff][cc.ss+1]!='.'&&grid[cc.ff][cc.ss-1]!='.'&&visited[cc.ff][cc.ss]==false)ans++;
                    grid[cc.ff][cc.ss]='#';
                    visited[cc.ff][cc.ss]=true;
                    if(grid[cc.ff-1][cc.ss]=='.'&&visited[cc.ff-1][cc.ss]==false){
                        to_go.push(mp(cc.ff-1,cc.ss));
                    }
                    if(grid[cc.ff+1][cc.ss]=='.'&&visited[cc.ff+1][cc.ss]==false){
                        to_go.push(mp(cc.ff+1,cc.ss));
                    }
                    if(grid[cc.ff][cc.ss+1]=='.'&&visited[cc.ff][cc.ss+1]==false){
